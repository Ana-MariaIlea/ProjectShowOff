...
public class TextLocaliserEditWindow : EditorWindow
{
    public static void Open(string key)
    {
        ...
        window.titleContent = new GUIContent("localiser Content");
        window.ShowUtility();
        window.key = key;
    }
...
public class TextLocaliserSearchWindow : EditorWindow
{
    public static void Open()
    {
        ...
        window.titleContent = new GUIContent("localiser Search");
        window.ShowUtility();
    ...
    private void GetSearchResults()
    {
        ...
        foreach (KeyValuePair<string,string> element in dictionary)
        {
            if(element.Key.ToLower().Contains(value.ToLower())|| element.Value.ToLower().Contains(value.ToLower()))
            {
                ...
                if(GUILayout.Button(content, GUILayout.MaxHeight(20)))
                {
                    if(EditorUtility.DisplayDialog("Remove key "+element.Key+"?","Tjis will remove elemeny from localisation, are you sure?","Do it"))
                    {
